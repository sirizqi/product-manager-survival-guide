...
**6. Open Questions:** Be honest about what's still unresolved. Pretending you have all the answers when you don't just creates false confidence that unravels mid-development.
...
1. **Come prepared, not improvising live.** Have prioritized backlog items ready with enough context (linked to PRDs, designs) that the team isn't discovering requirements for the first time during the planning meeting itself.
...
1. **Limit to genuinely actionable items.** Instead of ten vague observations, walk away with 1-3 specific, owned action items with a clear person responsible and a follow-up date.
...
**Why this is dangerous:** Every addition, however small it seems, extends timeline, introduces new edge cases, and often cascades into further scope additions ("well, if we're adding X, we should probably also handle Y").

**A healthier process:**

1. **Capture the idea, don't immediately act on it.** "That's a good idea—let's add it to the backlog for next sprint rather than expanding scope on something already in flight."
...
In your next retrospective, before generating new discussion topics, first revisit action items from your _previous_ retro. Were they actually implemented? If not, why not? This single practice will immediately reveal whether your retros are generating real change or just performing the ritual of reflection.
